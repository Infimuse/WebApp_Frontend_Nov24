                ...
                <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75 z-50">
                    <div className="bg-white rounded-lg w-full max-w-lg p-4 relative">
                        ...
                        <div className="mt-2">
                            {selectedGif && (
                                <div className="flex items-center space-x-2">
                                    <img src={selectedGif} alt="Selected GIF" className="w-24 h-24 object-cover rounded" />
                                    <button onClick={() => setSelectedGif(null)} className="text-red-500">Remove GIF</button>
                                </div>
                            )}
                            {selectedImage && (
                                <div className="flex items-center space-x-2">
                                    <img src={selectedImage} alt="Selected" className="w-24 h-24 object-cover rounded" />
                                    <button onClick={() => setSelectedImage(null)} className="text-red-500">Remove Image</button>
                                </div>
                            )}
                            {selectedDate && (
                                <div className="flex items-center space-x-2">
                                    <span>{`Scheduled for: ${selectedDate}`}</span>
                                    <button onClick={() => setSelectedDate(null)} className="text-red-500">Remove Date</button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex justify-between items-center mt-4">
                            ...
                            {/* Post Button */}
                            <button
                                className={`py-2 px-4 rounded-full text-white font-bold ${postContent || selectedGif || selectedImage || selectedDate
                                    ? 'bg-blue-600 hover:bg-blue-700'
                                    : 'bg-gray-400 cursor-not-allowed'}`}
                                onClick={handlePostSubmit}
                                disabled={!postContent && !selectedGif && !selectedImage && !selectedDate}
                            >
                                Post
                            </button>
                        </div>
